...
class SampleCheckpoint(ModelCheckpoint):
    ...
    def on_train_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule, *args, **kwargs):
        if self.every >=1 and (trainer.current_epoch +1) % self.every == 0:
            ...
            current = Path(self.dirpath) / f"yolov3_checkpoint_epoch_{trainer.current_epoch}.ckpt"
            trainer.save_checkpoint(current) 


            
def train_yolov3_model(model, datamodule, ckpt_path=None, epochs = 2, precision=16):
    trainer = Trainer(
        ...
        callbacks=[LearningRateMonitor(logging_interval="step"), 
                   ...
                   SampleCheckpoint(dirpath="logs/",every=5, verbose=True)],
